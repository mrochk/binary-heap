...
template <typename T> class Heap {
   public:
    ...
    /* Creates a new empty heap that uses the compare() function to compare
     * elements. */
   ...
   private:
    ...
    static auto constexpr compare = [](T a, T b) { return a > b; };
...
void Heap<T>::_bubble_up(Node<T> *node, std::stack<Direction> dirlist) {
    ...
    if (dirlist.empty()) {
        if (compare(root_data, child_data)) swap_nodes(node, child);
        return;
    }
...
template <typename T> void Heap<T>::bubble_down(Node<T> *node) {
    ...
    if (node->right == EMPTY && node->left != EMPTY) {
        ...
        child       = node->left;
        if (compare(root_data, left_data)) swap_nodes(node, child);
        return;
    }
    ...
    if (!compare(root_data, comp_with_root)) return;

    if (compare(right_data, left_data)) dir = L;
    else dir = R;
...
template <typename T> T Heap<T>::get_top() {
    return (tree == EMPTY) ? UINT_MAX : this->tree->data;
}
